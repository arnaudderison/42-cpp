...
ClapTrap::ClapTrap(std::string name, unsigned int hitPoint, unsigned int energyPoint, unsigned int damage) : _name(name) , _hitPoint(hitPoint), _energyPoint(energyPoint), _damage(damage) {
    std::cout << YELLOW << "[INFO] " << RESET;
    std::cout << "Constructor called and init" << std::endl;
    std::cout << "ClapTrap with " << GREEN << name << RESET << " name created" << std::endl;
}
